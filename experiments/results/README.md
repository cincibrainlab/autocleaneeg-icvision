...
| `2026-08-20_sol_tightened_v1_full679.csv` | `gpt-5.6-sol` + `prompts/tightened_v1_strip.txt`, **full 679-set** — graduated from the screening pass above | **69.07% (469/679), subject-clustered 95% CI [64.9%, 79.2%] — highest and tightest result of any configuration tested this session.** Overlaps both `terra`'s and (now that it has its own interval, see below) ICLabel's CI |
| `2026-08-20_iclabel_full679.csv` | MNE-ICLabel, full 679-set, deterministic/local/zero-cost — the reference classifier, reproduced from `/tmp/icvision_reconstructed_baseline/iclabel_full_baseline.csv` on cblprod (same saved EEGLAB ICA decompositions, same 679 components, same ground truth). Original file used a short-form label vocabulary (`"other"`, `"channel"`) for both true and predicted labels; converted to icvision's full-name vocabulary (`"other_artifact"`, `"channel_noise"`) via the same normalization used throughout this project, verified accuracy-invariant (448/679 = 65.98% before and after) | 65.98%, subject-clustered 95% CI **[62.4%, 76.9%]** — first interval ever computed for ICLabel in this document |
...
**Update, this gap is now closed**: ICLabel's raw predictions are committed above with their own subject-clustered CI, [62.4%, 76.9%]. This meaningfully changes how "beats ICLabel" claims elsewhere in this document should be read — `sol` + `tightened_v1`'s interval [64.9%, 79.2%] overlaps ICLabel's substantially, and their per-subject means are close (72.1% vs 69.6%). Treat any "X beats ICLabel" statement as directional, not settled, until stated otherwise.
