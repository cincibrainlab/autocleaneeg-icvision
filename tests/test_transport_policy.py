"""Offline Gate 0 transport-policy tests."""
...
@pytest.mark.parametrize(
    ...
    [
        ("raw", "", "openai-responses", "Raw transport does not accept base_url."),
        (
            "raw",
            None,
            None,
            "Raw transport requires endpoint_profile='openai-responses'.",
        ),
        (
            "raw",
            None,
...
def test_raw_selection_returns_only_reviewed_profile():
    selection = validate_transport_policy("raw", None, "openai-responses")
    assert selection.endpoint_profile is OPENAI_RESPONSES_PROFILE
    ...
    assert raw_credential_environment(selection) == "OPENAI_API_KEY"


def test_registry_and_profile_are_immutable():
    assert isinstance(ENDPOINT_PROFILES, MappingProxyType)
    with pytest.raises(TypeError):
        ENDPOINT_PROFILES["another"] = OPENAI_RESPONSES_PROFILE
    with pytest.raises(FrozenInstanceError):
        OPENAI_RESPONSES_PROFILE.host = "marker.invalid"


def test_raw_credential_source_rejects_unreviewed_equal_profile():
    counterfeit = replace(OPENAI_RESPONSES_PROFILE)
...
def test_ambient_endpoint_proxy_and_bundle_variables_do_not_change_raw_profile():
    environ = {
...
def test_core_raw_gate_precedes_all_pipeline_work(monkeypatch):
    ...
    with pytest.raises(RuntimeError, match=r"^Raw transport is not enabled in Gate 0\.$"):
        core.label_components(object(), transport="raw", endpoint_profile="openai-responses")


def test_core_invalid_transport_precedes_all_pipeline_work(monkeypatch):
    ...
    with pytest.raises(TransportPolicyError):
        core.label_components(
            ...
            transport="raw",
            base_url="",
            endpoint_profile="openai-responses",
        )
