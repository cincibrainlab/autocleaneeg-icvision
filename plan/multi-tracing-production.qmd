...
### CLIProxy Endpoint Testing and Reasoning Effort Analysis

Extensive testing was performed to understand the behavior of different endpoints and reasoning effort settings.

#### Reasoning Effort Parameter Behavior

The CLIProxy translates the `reasoning_effort` parameter to OpenAI's `reasoning.effort` field. When no parameter is provided, the proxy defaults to `"medium"`.

**Supported values for gpt-5.2**: `none`, `low`, `medium`, `high`, `xhigh`

**Final Test Results (Production, Patched)**:

| Reasoning | Time | Artifacts | Notes |
|-----------|------|-----------|-------|
| `none` | **50.60s** | 7 | **Fastest** |
| Default (→medium) | 56.11s | 5 | Proxy default |
| `low` | 81.36s | 4 | Slowest (paradoxically) |
| `minimal` | ERROR | - | Not supported by gpt-5.2 |

::: {.callout-important}
## Key Finding: `low` Reasoning is Slower Than Default

Counter-intuitively, `reasoning_effort='low'` results in **slower** API responses than the default (`medium`). This is not a bug but appears to be OpenAI API behavior:

1. **Proxy default**: When no `reasoning_effort` is set, CLIProxy sends `reasoning.effort: "medium"` to OpenAI
2. **OpenAI optimization**: The `"medium"` path appears to be optimized for vision tasks
3. **`low` penalty**: Explicitly requesting `"low"` may trigger a different, slower code path

**Root cause** (from CLIProxy team): The proxy's translator always sets reasoning.effort - either from your request or defaulting to `"medium"`. OpenAI's Responses API appears to have optimized the `"medium"` path for Codex/vision workflows.
:::

::: {.callout-tip}
## Recommendations for Reasoning Effort

| Use Case | Setting | Expected |
|----------|---------|----------|
| **Fastest processing** | `--reasoning-effort none` | ~50s, 7 artifacts |
| **Balanced (default)** | Don't set parameter | ~56s, 5 artifacts |
| **Avoid** | `--reasoning-effort low` | ~81s, slowest |

For production pipelines, either use **no parameter** (proxy default) or explicitly set `none` for maximum speed.
:::

#### Endpoint Evolution

Testing revealed behavior changes as CLIProxy was patched:

| Phase | Endpoint | Default Time | Artifacts |
|-------|----------|--------------|-----------|
| Initial | Production | 51.24s | 5 |
| Broken | Production | 68.58s | 6 |
| Local fix v1 | localhost:28080 | 68.54s | 7 |
| Local fix v2 | localhost:28080 | 49.55s | 7 |
| **Final (patched)** | **Production** | **56.11s** | **5** |

The production endpoint is now stable with the vision routing fix deployed.
