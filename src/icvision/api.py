...
def classify_component_image_openai(
    ...
    try:
        ...
        if resp_text:
            logger.debug(f"Raw OpenAI response for {image_path.name}: '{resp_text[:100]}...'")
            ...
            if match:
                ...
                logger.debug(f"Parsed classification for {image_path.name}: Label={label}, Conf={confidence:.2f}")
                return label, confidence, reason
            else:
                logger.warning(f"Could not parse OpenAI response for {image_path.name}: '{resp_text}'. Defaulting.")
        ...
        logger.error(f"OpenAI API status error for {image_path.name}: Status={e.status_code}, Response={e.response}")
        ...
        logger.error(f"Unexpected error during vision classification for {image_path.name}: {type(e).__name__} - {e}")
        return "other_artifact", 1.0, f"Unexpected Exception: {type(e).__name__} - {str(e)[:100]}"


def _classify_single_component_wrapper(args_tuple: Tuple[int, Optional[Path], str, str, Optional[str]]) -> Tuple[int, str, float, str]:
...
def classify_components_batch(
    ...
    auto_exclude: bool = DEFAULT_CONFIG["auto_exclude"],
    ...
    try:
        ...
        for i in range(num_components):
            try:
                ...
                logger.warning(f"Failed to plot component IC{i}: {plot_err}. Will mark as plotting_failed.")
                component_plot_args.append((i, None, api_key, model_name, custom_prompt))
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_concurrency) as executor:
            ...
            for future in concurrent.futures.as_completed(futures):
                try:
                    ...
                    logger.error(f"Error processing a component future: {e}. Defaulting result.")
                ...
                log_level = "debug" if label == "brain" and not exclude_this_component else "warning" if exclude_this_component else "info"
                logger.log(logging.getLevelName(log_level.upper()), 
                           f"IC{comp_idx:03d} | Label: {label.upper():<13} (MNE: {mne_label:<10}) | Conf: {confidence:.2f} | Excl: {exclude_this_component}")
                processed_count += 1

        logger.info(f"OpenAI classification complete. Processed {processed_count}/{num_components} components.")
